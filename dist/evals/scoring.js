...
export function scoreScenarioRules(subject, scenario) {
    ...
    const taskCompletion = clampScore((scenario.mustContain.length === 0 ? 1 : contentHitCount / scenario.mustContain.length)
        - forbiddenContentHits.length * 0.3
        - (subject.error ? 0.4 : 0));
    const finalAnswerQuality = clampScore((subject.finalOutput.trim() ? 1 : 0)
...
export function scoreOnlineRules(subject) {
...
export async function scoreWithLlmJudge(subject, options) {
